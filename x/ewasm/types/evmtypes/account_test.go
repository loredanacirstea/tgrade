...
import (
	...
	ethermintcodec "github.com/confio/tgrade/encoding/codec"
...
func (suite *AccountTestSuite) SetupTest() {
	...
	suite.account = &evmtypes.EthAccount{
		BaseAccount: baseAcc,
...
func (suite *AccountTestSuite) TestAccountType() {
	...
	suite.Require().Equal(evmtypes.AccountTypeEOA, suite.account.Type())
	suite.account.CodeHash = common.BytesToHash(crypto.Keccak256([]byte{1, 2, 3})).Hex()
	suite.Require().Equal(evmtypes.AccountTypeContract, suite.account.Type())
}
