...
func newDynamicFeeTx(tx *ethtypes.Transaction) (*DynamicFeeTx, error) {
	...
	if tx.Value() != nil {
		amountInt, err := evmtypes.SafeNewIntFromBigInt(tx.Value())
		if err != nil {
			return nil, err
		}
	...
	if tx.GasFeeCap() != nil {
		gasFeeCapInt, err := evmtypes.SafeNewIntFromBigInt(tx.GasFeeCap())
		if err != nil {
			return nil, err
		}
	...
	if tx.GasTipCap() != nil {
		gasTipCapInt, err := evmtypes.SafeNewIntFromBigInt(tx.GasTipCap())
		if err != nil {
			return nil, err
		}
...
func (tx DynamicFeeTx) Validate() error {
	...
	if !evmtypes.IsValidInt256(tx.GetGasTipCap()) {
		return sdkerrors.Wrap(ErrInvalidGasCap, "out of bound")
	}

	if !evmtypes.IsValidInt256(tx.GetGasFeeCap()) {
		return sdkerrors.Wrap(ErrInvalidGasCap, "out of bound")
	}
	...
	if !evmtypes.IsValidInt256(tx.Fee()) {
	...
	if !evmtypes.IsValidInt256(amount) {
		return sdkerrors.Wrap(ErrInvalidAmount, "out of bound")
	}

	if tx.To != "" {
		if err := evmtypes.ValidateAddress(tx.To); err != nil {
