...
var _ evmtypes.EVMTxIndexer = &KVIndexer{}
...
func (kv *KVIndexer) IndexBlock(block *tmtypes.Block, txResults []*abci.ResponseDeliverTx) error {
	...
	for txIndex, tx := range block.Txs {
		...
		for msgIndex, msg := range tx.GetMsgs() {
			ethMsg := msg.(*types.MsgEthereumTx)
			txHash := common.HexToHash(ethMsg.Hash)

			txResult := evmtypes.TxResultIndexer{
				Height:     height,
...
func (kv *KVIndexer) GetByTxHash(hash common.Hash) (*evmtypes.TxResultIndexer, error) {
	...
	var txKey evmtypes.TxResultIndexer
...
func (kv *KVIndexer) GetByBlockAndIndex(blockNumber int64, txIndex int32) (*evmtypes.TxResultIndexer, error) {
...
func saveTxResult(codec codec.Codec, batch dbm.Batch, txHash common.Hash, txResult *evmtypes.TxResultIndexer) error {
