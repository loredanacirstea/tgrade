...
import (
	...
	cryptocodec "github.com/confio/tgrade/crypto/codec"
...
func TestKeyring(t *testing.T) {
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, evmtypes.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, evmtypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(evmtypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Ewasm implementation
	...
	// Equality of Eth and Ewasm implementation
	...
	// Inequality of wrong derivation path of Eth and Ewasm implementation
